...
struct filesystem fat16_fs = {
    .resolve = fat16_resolve, .open = fat16_open, .read = fat16_read};
...
struct fat_directory_item* fat16_clone_directory_item(
    struct fat_directory_item* item, int size) {
  struct fat_directory_item* item_copy = 0;
  if (size < sizeof(struct fat_directory_item)) return 0;
...
static int fat16_cluster_to_sector(struct fat_private* private, int cluster) {
  return private->root_directory.ending_sector_pos +
         ((cluster - 2) * private->header.primary_header.sector_per_cluster);
}
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
  ...
  res = diskstreamer_seek(
      stream, fat_table_position * (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE));
  if (res < 0) goto out;
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster,
  ...
  int size_of_cluster_bytes =
      private->header.primary_header.sector_per_cluster * disk->sector_size;
...
static int fat16_read_internal_from_stream(struct disk* disk,
  ...
  if (res != PEACHOS_ALL_OK) goto out;
...
struct fat_directory* fat16_load_fat_directory(
    struct disk* disk, struct fat_directory_item* item) {
  int res = 0;
...
out:
  if (res != PEACHOS_ALL_OK) fat16_free_directory(directory);
  return directory;
}

struct fat_item* fat16_new_fat_item_for_directory_item(
    struct disk* disk, struct fat_directory_item* item) {
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk,
  ...
  for (int i = 0; i < directory->total; i++) {
    fat16_get_full_relative_filename(&directory->item[i], tmp_filename,
                                     sizeof(tmp_filename));
    if (istrncmp(tmp_filename, name, strlen(tmp_filename)) == 0) {
...
void* fat16_open(struct disk* disk, struct path_part* path, FILE_MODE mode) {
  if (mode != FILE_MODE_READ) return ERROR(-EINVARG);
